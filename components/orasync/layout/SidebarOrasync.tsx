...
const navigationItems = [
  ...
  { href: '/unified-inbox', label: 'Inbox', icon: Mail },
  { href: '/ai-chatbot', label: 'AI Chatbot', icon: Zap },
  { href: '/reputation-management', label: 'Reputation', icon: Star },
  { href: '/analytics', label: 'Analytics', icon: BarChart3 },
]

export default function SidebarOrasync() {
    ...
    <>
      ...
      >
        ...
        <div className="px-4 pb-6 border-b border-sidebar-border">
          <Link href="/orasync-dashboard" className="flex items-center gap-2">
            <OrasyncLogo className="w-8 h-8" textClassName="text-lg" />
          </Link>
        </div>
        ...
        <nav className="flex-1 space-y-2 px-4 py-6">
          {navigationItems.map(({ href, label, icon: Icon }) => {
            const isActive =
              pathname === href ||
              (href !== '/orasync-dashboard' && pathname.startsWith(href.split('/')[1]))
